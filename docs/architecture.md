# BC-Analyzer — System Architecture
...
15. [Report Output Structure](#15-report-output-structure)
...
│                                                  │    │  Structured logging → logs/agent_runs.log      │
│                                                  │    │                                                │
└──────────────────────┬───────────────────────────┘    └────────────────────┬───────────────────────────┘
                       │                                                     │
                       └────────────────┐  ┌─────────────────────────────────┘
                       ...
                       FastAPI (main.py :8000)
                         ...
                         └── POST /api/approve    ←── Step 2: Run full agentic pipeline
                            ...
                            LangGraph Orchestrator
                                   │
                                   ▼
                            logs/agent_runs.log  ←── Structured execution logs
```
...
│   │   ├── UserDashboard.tsx
│   │   ├── AdminDashboard.tsx
│   │   ├── IdeaEntry.tsx               # Main pipeline UI (2-step + report)
│   │   ├── CapabilityDashboard.tsx     # BIZBOK capability editor
│   │   ├── JourneyDashboard.tsx        # Journey mapping
│   │   └── StrategyDashboard.tsx       # Value streams + strategy
│   ├── lib/
...
├── logs/
│   └── agent_runs.log                   # Structured agent execution logs
└── app/
...
class AnalyzerState(TypedDict):
    ...
    _run_id: str                 # Logging correlation ID
```
...
```bash
# Tail live logs
tail -f backend/logs/agent_runs.log

# Filter by run ID
grep "a3f7c1b2" backend/logs/agent_runs.log

# Filter QA failures only
grep "QA FAILED" backend/logs/agent_runs.log
```
